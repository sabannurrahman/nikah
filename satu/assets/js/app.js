...
document.addEventListener('DOMContentLoaded', function() {
    ...
    */
    ...
    btn_open.addEventListener('click', function(e) {
        ...
        const section1SpouseText = document.querySelector('#section-1 h1');
        section1SpouseText.innerHTML = section1SpouseText.textContent.replace(/\S/g, "<span class='letter'>$&</span>");
        anime.timeline().add({
    ...
    */
    ...
    glideSection.mount();
    // glide : ucapan slide & form toggling between form section and ucapan section
    const btn_section2_ucapan = document.querySelector('#btn-section-2-container-ucapan');
    const btn_section2_form = document.querySelector('#btn-section-2-container-form');
    const section2_form = document.querySelector('#section-2-container-form');
    const section2_ucapan = document.querySelector('#section-2-container-ucapan');
    const btn_section2_ucpanan_next = section2_ucapan.querySelector('[data-glide-dir=">"]');
    const btn_section2_ucpanan_prev = section2_ucapan.querySelector('[data-glide-dir="<"]');
    let glideUcapan = null;
    btn_section2_ucapan.addEventListener('click', function(e) {
        section2_form.classList.add('d-none');
        section2_ucapan.classList.remove('d-none');
        glideUcapan = new Glide('#section-2-container-ucapan', {
            type: 'carousel',
            perView: 1,
            focusAt: 'center',
        })

        btn_section2_ucpanan_next.addEventListener('click', function() {
            glideUcapan.go('>');
        })
        btn_section2_ucpanan_prev.addEventListener('click', function() {
            glideUcapan.go('<');
        })

        glideUcapan.mount();
    });
    btn_section2_form.addEventListener('click', function(e) {
        section2_ucapan.classList.add('d-none');
        section2_form.classList.remove('d-none');
        let slideHeight = document.querySelector('.glide__slide--active').offsetHeight;
        let glideTrack = document.querySelector('.glide__track').offsetHeight;
        if (slideHeight != glideTrack) {
            let newHeight = slideHeight;
